...
function SportsDetail() {
  ...
  const [sportsDetails, setSportsDetails] = useState(); // Corrected typo in useState variable
  ...
  useEffect(() => {
    ...
    axios.get(`/api/sports/${sport_name}`)
      .then(response => {
        ...
        dispatch({ type: 'FETCH_SPORTS', payload: response.data }); // Update Redux state with fetched sport details
      })
      .catch(error => {
